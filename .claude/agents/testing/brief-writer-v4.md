# Brief Writer v4 — Headline, Subheadline & Citation Lab
...
The headline is built from the collision between these elements:
...
The headline's job is to sell the **collision point** — the most
surprising or compelling angle where these elements meet. It does NOT
need to state all three explicitly. It must contain at least two, and
the third can be implied or picked up by the subheadline. Prioritize
editorial sharpness over ingredient completeness.

Between the headline AND the subheadline together, all three ingredients
...
- Never build the headline from wave/research findings. No statistics,
  percentages, or survey data in the headline. Wave content belongs in
  the zeitgeist section, not the headline.
...
**Headline craft process:**
Generate 5 candidate headlines, each using a DIFFERENT editorial
structure. Consider approaches like:
- A tension or paradox that demands resolution
- A provocative claim that reframes what the reader thought they knew
- A concrete, specific detail that opens a larger story
- An inversion — flipping the expected framing
- A question that the journalist would want to answer
- A declaration that names a shift

Do not default to "כש-X — Y מקבל מענה Z" or any other single formula.
After generating all 5, select the one that would most make a journalist
in `{{company_industry}}` stop scrolling and want to pitch this story
to their editor.

Output only the winning headline in the final brief. Include all 5
candidates in a `## מועמדים לכותרת` section at the end of the file
(after the main output) so the client can review alternatives.

**The litmus test:** Does the headline sell a compelling collision point
built from at least two of the three mandatory ingredients? Would a
journalist read it and want to pitch the story to their editor? If it
feels like a template or a press release — rewrite. Then verify: between
the headline and the subheadline together, are all three ingredients
clearly present?
...
Save to: `clients/{{company_id}}/launches/{{product_id}}/briefs-v4/headline_test_{{timestamp}}.md`
If the `briefs-v4/` folder does not exist, create it before saving.
...
2. **5 candidates generated** with different structures — no two share the same pattern
3. **Subheadline contains** company_name, product_name, target_audience, and mechanism
4. **Quote does NOT repeat** headline or subheadline content
5. **Quote matches** spokesperson_speaking_style
6. **No forbidden words** anywhere
7. **Language** matches input variables
